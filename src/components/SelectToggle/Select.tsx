...
export default function Select({
    ...
    return (
        <OutsideClickDetector callback={handleOutsideClick}>
            ...
            >
                <button
                    className="flex flex-wrap items-center justify-between gap-2 whitespace-nowrap rounded-lg 
                                bg-zinc-200 px-2 py-1.5 text-start text-sm text-gray transition-all duration-300 
                                hover:bg-zinc-400 hover:text-white"
                >
                    {selected?.label || placeholder}
                    <BsArrow90DegDown
                        className="min-h-[15px] min-w-[15px] transition-all duration-75 
                                                group-data-[open=true]:rotate-180"
                    />
                </button>
                {isOpen && (
                    <div
                        className="dropdown absolute left-0 top-9 rounded-md bg-white py-1.5 text-sm text-gray 
                                    shadow-lg"
                    >
                        {children}
                    </div>
